...
exports.register = async (id, name, org, pwph, totpSecret) => {
	return new Promise(async resolve => {
		...
		usersDB.run(`INSERT INTO users(id, name, org, pwph, totpsec, approved) VALUES (?,?,?,?,?,?)`, 
			[id,name,org,pwph,totpSecret,0], err => err?resolve({result:false, err}):resolve({result:true}));
	});
}
...
exports.exists = exports.login = pwph => {
	return new Promise(resolve => {
		...
		.then(pwph => {
			usersDB.all(`SELECT id, name, org, totpsec, approved FROM users WHERE pwph = ? COLLATE NOCASE;`, [pwph], (err, rows) => {
				if (err || !rows.length) resolve({result: false, err});
				else if (!rows[0].approved) resolve({result: false, err:"Not approved"});
