...
module.exports.exec = async function(params) {
    ...
    if(roleman.isOrgAdminLoggedIn()) {  // if org admin then just add the image to the org data, no need to update hosts
        const addImgToOrgData = await dbAbstractor.addOrgResource(roleman.getCurrentOrg(), imgname, imguri, 
        processorarchitecture.toLowerCase(), description||"", extrainfo||"", VMIMAGE);
        if (!addImgToOrgData) {
