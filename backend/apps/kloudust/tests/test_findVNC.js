...
/**
 * Tests finding VNC for a given VM.
 * 
...
const COMMAND_NAME = "findVNC";

exports.runTestsAsync = async function(argv) {
    if ((!argv[0]) || (argv[0] != COMMAND_NAME)) {
        LOG.console(`Skipping FIND VNC test case, not called.\n`);
        return;
    }
    ...
    LOG.console(`Running FIND VNC test case, command: ${COMMAND_NAME} for ${argv.map(a => `"${a}"`).join(" ")}\n`);
    ...
    if (result && result.result) {
        LOG.console(`VNC found successfully for VM ${argv[1]}.\nVNC LOG: ${result.stdout}\n`);
        return true;
    } else {
        LOG.console(`Failed to find the VNC for VM ${argv[1]}. Error: ${result.stderr}\n`);
        return false;
    };
}
