...
const COMMAND_NAME = "cloneVM";

exports.runTestsAsync = async function(argv) {
    if ((!argv[0]) || (argv[0] != COMMAND_NAME)) {
        LOG.console(`Skipping CLONE VM test case, not called.\n`);
        return;
    }
    
    if(!argv[1] || !argv[2]) {
        LOG.console(`Invalid parameters for ${COMMAND_NAME} test case.\n`);
        LOG.console(`Usage: ${COMMAND_NAME} <existing vm name> <clone vm name>\n`);
        return;
    }

    LOG.console(`Running CLONE VM test case, command: ${COMMAND_NAME} for ${argv.map(a => `"${a}"`).join(" ")}\n`);
    ...
    if (result && result.result) {
        LOG.console(`VM ${argv[2]} cloned successfully from existing VM ${argv[1]}.\n`);
        return true;
    } else {
        LOG.console(`Failed to clone VM ${argv[1]}. Error: ${result.stderr}\n`);
        return false;
    };
}
