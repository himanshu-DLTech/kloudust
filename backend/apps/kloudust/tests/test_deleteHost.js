...
const COMMAND_NAME = "deleteHost";

exports.runTestsAsync = async function(argv) {
    if ((!argv[0]) || (argv[0] != COMMAND_NAME)) {
        LOG.console(`Skipping DELETE HOST test case, not called.\n`);
        return;
    }
    
    if(!argv[1]) {
        LOG.console(`Invalid parameters for ${COMMAND_NAME} test case.\n`);
        LOG.console(`Usage: ${COMMAND_NAME} <host name> [clean host machine]\n`);
        return;
    }

    LOG.console(`Running DELETE HOST test case, command: ${COMMAND_NAME} for ${argv.map(a => `"${a}"`).join(" ")}\n`);
    ...
    if (result && result.result) {
        LOG.console(`HOST ${argv[1]} deleted successfully.\n`);
        return true;
    } else {
        LOG.console(`Failed to delete the HOST ${argv[1]}. Error: ${result.stderr}\n`);
        return false;
    };
}
