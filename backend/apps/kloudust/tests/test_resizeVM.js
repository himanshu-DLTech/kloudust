...
const COMMAND_NAME = "resizeVM";

exports.runTestsAsync = async function(argv) {
    if ((!argv[0]) || (argv[0] != COMMAND_NAME)) {
        LOG.console(`Skipping RESIZE VM test case, not called.\n`);
        return;
    }
    
    if(!argv[1]) {
        LOG.console(`Invalid parameters for ${COMMAND_NAME} test case.\n`);
        LOG.console(`Usage: ${COMMAND_NAME} <vm name> [cores] [memory] [disk_new_gb] [disk_name] [inplace_disk_resize] [restart]\n`);
        return;
    }

    LOG.console(`Running RESIZE VM test case, command: ${COMMAND_NAME} for ${argv.map(a => `"${a}"`).join(" ")}\n`);
    ...
    if (result && result.result) {
        LOG.console(`VM ${argv[1]} resized successfully.\n`);
        return true;
    } else {
        LOG.console(`Failed to resize the VM ${argv[1]}. Error: ${result.stderr}\n`);
        return false;
    };
}
