...
const COMMAND_NAME = "deleteVM";

exports.runTestsAsync = async function(argv) {
    if ((!argv[0]) || (argv[0] != COMMAND_NAME)) {
        LOG.console(`Skipping DELETE VM test case, not called.\n`);
        return;
    }
    ...
    LOG.console(`Running DELETE VM test case, command: ${COMMAND_NAME} for ${argv.map(a => `"${a}"`).join(" ")}\n`);
    ...
    if (result && result.result) {
        LOG.console(`VM ${argv[1]} deleted successfully.\n`);
        return true;
    } else {
        LOG.console(`Failed to delete the VM ${argv[1]}. Error: ${result.stderr}\n`);
        return false;
    };
}
