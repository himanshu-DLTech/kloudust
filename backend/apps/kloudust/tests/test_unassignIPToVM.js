...
/**
 * Tests unassigning a given IP to the VM.
 * 
...
const COMMAND_NAME = "unassignIPToVM";

exports.runTestsAsync = async function(argv) {
    if ((!argv[0]) || (argv[0] != COMMAND_NAME)) {
        LOG.console(`Skipping UNASSIGN VM IP test case, not called.\n`);
        return;
    }
    ...
    LOG.console(`Running UNASSIGN VM IP test case, command: ${COMMAND_NAME} for ${argv.map(a => `"${a}"`).join(" ")}\n`);
    ...
    if (result && result.result) {
        LOG.console(`IP ${argv[2]} unassigned successfully to VM ${argv[1]}.\n`);
        return true;
    } else {
        LOG.console(`Failed to unassign IP ${argv[2]} to VM ${argv[1]}. Error: ${result.stderr}\n`);
        return false;
    };
}
