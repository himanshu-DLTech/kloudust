...
const COMMAND_NAME = "rebootHost";

exports.runTestsAsync = async function(argv) {
    if ((!argv[0]) || (argv[0] != COMMAND_NAME)) {
        LOG.console(`Skipping REBOOT HOST test case, not called.\n`);
        return;
    }
    ...
    LOG.console(`Running REBOOT HOST test case, command: ${COMMAND_NAME} for ${argv.map(a => `"${a}"`).join(" ")}\n`);
    ...
    if (result && result.result) {
        LOG.console(`HOST ${argv[1]} is successfully rebooted\n`);
        return true;
    } else {
        LOG.console(`Failed to reboot the HOST ${argv[1]}. Error: ${result.stderr}\n`);
        return false;
    };
}
